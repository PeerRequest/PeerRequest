...
public class  PeerRequestBackend {
    ...
    private static void mockData(ConfigurableApplicationContext context) {
        ...
        for (var user : users.stream().toList()) {
            if (user.firstName().equals("User2")) {
                names[ind] = "User2";
                userIds[ind] = user.getId();
                ind++;
            }
            if (user.firstName().equals("User1")) {
                names[ind] = "User1";
                userIds[ind] = user.getId();
                ind++;
            }
            if (user.firstName().equals("User3")) {
                names[ind] = "User3";
                userIds[ind] = user.getId();
                ind++;
            }
            if (user.firstName().equals("User4")) {
                names[ind] = "User4";
                userIds[ind] = user.getId();
                ind++;
            }
            if (user.firstName().equals("User5")) {
                names[ind] = "User5";
                userIds[ind] = user.getId();
                ind++;
            }

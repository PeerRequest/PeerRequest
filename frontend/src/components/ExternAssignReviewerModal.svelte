...
<Modal bind:open={show} on:hide={() => hide ? hide() : null} title="Assign Reviewers">
  <div class="flex flex-row justify-between items-center">
    ...
    <ButtonGroup>
      <Button color={strategy === "bidding" ? "primary" : "alternative"} on:click={() => strategy = "bidding"}>Bidding
      </Button>
      <Button color={strategy === "direct" ? "primary" : "alternative"} on:click={() => strategy = "direct"}>Direct
        Assignment
      </Button>
    </ButtonGroup>
  </div>
  ...
  <svelte:fragment slot="footer">
    <Button color="primary"
            on:click={() => result(strategy === "direct", strategy === "direct" ? Object.fromEntries(Object.entries(paper_matches).filter(([key, value]) => reviewers.some(r => value && key.startsWith(r.id.toString())))) : {})}>{strategy === "direct" ? "Assign" : "Start bidding"}</Button>
